"""Run the REAL pipeline for Riyadh — PARALLEL variant with domain-diverse
source approval + reuse-or-fresh discovery dispatch.

15 Riyadh-tailored categories: Saudi has rapidly opened to tourism since
Vision 2030 — Edge of the World (giant escarpment), Diriyah UNESCO site,
Al Masmak Fortress, desert/camel safaris, Riyadh Season winter events,
Bujairi Terrace, National Museum.

Usage: python run_riyadh_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("riyadh_parallel")

RIYADH_CITY_ID = "18fec2e9-164b-46a1-9cff-f3cfda2af081"
...
ALL_CATEGORIES = [
    ...
    "Cultural & Heritage",
    "Food & Drink",
    "Shopping & Traditional Souks",
    "Nature & Wildlife (Wadi Hanifah, Nofa Wildlife)",
    "Bujairi Terrace & Culinary Experiences",
    "Family & Kids",
    "Riyadh Season & Entertainment",
    "Luxury & Private",
    "Transfers",
]
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("RIYADH PARALLEL RUN — %d categories, max %d concurrent",
    ...
    try:
        async with async_session_factory() as db:
            row = await db.execute(text("SELECT count(*) FROM activities WHERE city = 'Riyadh'"))
            logger.info("\nTotal Riyadh activities in DB: %d", row.scalar())
    except Exception as exc:
