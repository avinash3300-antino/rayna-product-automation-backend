"""Run the REAL pipeline for Bali — PARALLEL-3 variant.

15 Bali-tailored categories tuned for the destination:
temples, Ubud/rice terraces, volcanoes, waterfalls, Nusa Penida, etc.

Usage: python run_bali_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("bali_parallel")

BALI_CITY_ID = "527af21f-61be-41b4-afaa-707e6567f857"
ADMIN_USER_ID = "b153d252-5802-47a4-8be7-0bb71a466127"

ALL_CATEGORIES = [
    "Sacred Temples & Cultural Sites",
    "Waterfalls & Nature",
    "Volcano Sunrise Hikes",
    "Beaches & Coastal Tours",
    "Water Sports & Rafting",
    "Cultural Shows & Dance",
    "Day Trips",
    "Transfers",
]
...
async def _run_category_once(category: str, idx: int, total: int, attempt: int):
    ...
    async with async_session_factory() as db:
        ...
        # Domain-diverse approval: pick top-1 per unique domain, up to 3.
        # Prevents TripAdvisor (JS-heavy, Jina Reader can't extract) from
        # hogging all 3 slots and blocking GYG/Viator/Klook which extract cleanly.
        ...
        seen: set[str] = set()
        to_approve = []
        for s in tier1:  # already sorted by (tier, authority_score DESC)
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("BALI PARALLEL RUN — %d categories, max %d concurrent",
    ...
    try:
        async with async_session_factory() as db:
            row = await db.execute(text("SELECT count(*) FROM activities WHERE city = 'Bali'"))
            logger.info("\nTotal Bali activities in DB: %d", row.scalar())
    except Exception as exc:
        logger.warning("Final count query failed (tunnel maybe down): %s", exc)
