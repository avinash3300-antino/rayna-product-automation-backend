"""Finish the Seoul run: recover 2 cats with existing discovery + fresh-run 10 cats.

The overnight run had 3 successes (Landmark 39, DMZ 29, Palace 27 = 95) then
network collapsed. This script tries recovery-first: if a category has an
existing successful discovery_run with sources > 0, reuse it. Otherwise
create fresh discovery (standard path). Domain-diverse approval + pipeline
in both cases.

Usage: python run_seoul_finish.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("seoul_finish")

SEOUL_CITY_ID = "3739f156-c962-4bcb-bb45-6eae2ef10cfc"
ADMIN_USER_ID = "b153d252-5802-47a4-8be7-0bb71a466127"

REMAINING_CATEGORIES = [
    "K-Pop & Entertainment Tours",              # has existing run (10 srcs)
    "K-Beauty & Shopping Tours",                # has existing run (9 srcs)
    "Nami Island & Petite France Day Trips",
...
async def _find_reusable_discovery_run(db, city_id, category):
    """Return the id of the most-recent completed discovery_run for
    (city_id, category) with sources_found > 0, else None."""
...
async def _run_category_once(category, idx, total, attempt):
    ...
    async with async_session_factory() as db:
        ...
        logger.info("[%d/%d] %d sources for %s", idx, total, len(sources), category)
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("SEOUL FINISH — %d categories, max %d concurrent (reuse-or-fresh discovery)",
    ...
    try:
        async with async_session_factory() as db:
            row = await db.execute(text("SELECT count(*) FROM activities WHERE city = 'Seoul'"))
            logger.info("\nTotal Seoul activities in DB: %d", row.scalar())
    except Exception as exc:
