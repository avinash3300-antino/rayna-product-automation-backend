"""Run the REAL pipeline for Tokyo — PARALLEL variant with domain-diverse
source approval (per pipeline-source-approval memory).

15 Tokyo-tailored categories: heavy focus on Mt Fuji/Hakone/Nikko day trips
(Tokyo's biggest tourism draws), anime/pop culture, tea ceremony, Disneyland,
Shibuya/Akihabara, plus airport transfers (Narita + Haneda).

Usage: python run_tokyo_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("tokyo_parallel")

TOKYO_CITY_ID = "64e49cb3-44f2-4445-acb4-b5c796344cfd"
ADMIN_USER_ID = "b153d252-5802-47a4-8be7-0bb71a466127"

ALL_CATEGORIES = [
    "Landmark Tickets",
    "Mount Fuji & Hakone Day Trips",
    "Nikko & Kamakura Day Trips",
    "Anime & Pop Culture Tours",
    "Traditional Culture & Tea Ceremony",
    "Museum & Gallery",
    "Sightseeing Tours",
    "Food & Drink",
    "Cultural & Heritage",
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("TOKYO PARALLEL RUN — %d categories, max %d concurrent (domain-diverse approval)",
    ...
    try:
        async with async_session_factory() as db:
            row = await db.execute(text("SELECT count(*) FROM activities WHERE city = 'Tokyo'"))
            logger.info("\nTotal Tokyo activities in DB: %d", row.scalar())
    except Exception as exc:
