"""Run the REAL pipeline for Seoul — PARALLEL variant with domain-diverse
source approval (per pipeline-source-approval memory).

15 Seoul-tailored categories: DMZ (unique tourist draw), K-Pop/K-Beauty tours,
5 palaces + hanbok, Nami Island day trips, Korean BBQ/street food, Everland,
Incheon transfers.

Usage: python run_seoul_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("seoul_parallel")
...
ALL_CATEGORIES = [
    "Landmark Tickets",
    "DMZ Tours",
    "Palace & Hanbok Experiences",
    "K-Pop & Entertainment Tours",
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("SEOUL PARALLEL RUN — %d categories, max %d concurrent (domain-diverse approval)",
