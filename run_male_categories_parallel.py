"""Run the REAL pipeline for Male / North Male Atoll (Maldives) — PARALLEL
variant with domain-diverse source approval + reuse-or-fresh discovery dispatch
(per pipeline-source-approval memory).

15 Maldives-tailored categories: heavy focus on snorkeling/diving (world-class),
sandbank picnics, manta ray & whale shark tours, seaplane transfers, resort
day passes, local island visits (Maafushi, Villimale, Guraidhoo).

Usage: python run_male_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("male_parallel")

MALE_CITY_ID = "f6e673e3-0786-4833-86a8-ddf9390c24aa"
ADMIN_USER_ID = "b153d252-5802-47a4-8be7-0bb71a466127"

ALL_CATEGORIES = [
    "Snorkeling & Diving",
    "Manta Ray & Whale Shark Tours",
    "Island Hopping & Local Islands",
    "Sandbank & Picnic Tours",
    "Sunset & Dolphin Cruise",
    "Water Sports & Adventure",
    "Submarine & Underwater Experiences",
    "Resort Day Passes",
    "Fishing Tours",
    "Male City & Cultural Tours",
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("MALE / NORTH MALE ATOLL PARALLEL RUN — %d categories, max %d concurrent",
    ...
    try:
        async with async_session_factory() as db:
            row = await db.execute(text("SELECT count(*) FROM activities WHERE city = 'Male'"))
            logger.info("\nTotal Male activities in DB: %d", row.scalar())
    except Exception as exc:
