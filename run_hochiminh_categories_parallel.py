"""Run the REAL pipeline for Ho Chi Minh City — PARALLEL variant with
domain-diverse source approval (baked in from lessons learned in Bali run).

15 HCMC-tailored categories:
- Cu Chi Tunnels & War History (iconic HCMC tours)
- Mekong Delta Day Trips
- Street Food & Motorbike Tours
- Vespa & Scooter Tours
- Water Puppet Shows / AO Show
- etc.

Usage: python run_hochiminh_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("hcmc_parallel")

HCMC_CITY_ID = "880a155b-c01b-4fc4-b422-b24c8580854d"
...
async def _run_category_once(category: str, idx: int, total: int, attempt: int):
    ...
    async with async_session_factory() as db:
        ...
        # Domain-diverse approval: top-1 per unique domain, up to 3.
        # Rule saved in memory (pipeline-source-approval).
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("HO CHI MINH CITY PARALLEL RUN — %d categories, max %d concurrent (domain-diverse approval)",
    ...
    try:
        async with async_session_factory() as db:
            row = await db.execute(text("SELECT count(*) FROM activities WHERE city = 'Ho Chi Minh City'"))
            logger.info("\nTotal HCMC activities in DB: %d", row.scalar())
    except Exception as exc:
