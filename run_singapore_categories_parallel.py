"""Run the REAL pipeline for Singapore — RESUME + PARALLEL-3 variant.

Runs the 10 categories that failed in the earlier tunnel-drop cascade.
Existing DB rows are deduped by activity_pipeline's existing name+city match;
source_urls[] gets merged, no duplicate rows.

Retries each category up to 3 times on connection errors — pairs with the
tunnel watchdog at /tmp/rayna-logs/tunnel-watchdog.sh which re-opens the
tunnel within ~5s of a drop.

Usage: python run_singapore_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("singapore_parallel")

SINGAPORE_CITY_ID = "4b070751-5578-4bd7-aa8e-70ae3ec6dc9a"
...
async def _run_category_once(category: str, idx: int, total: int, attempt: int):
    ...
    city_id = UUID(SINGAPORE_CITY_ID)
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("SINGAPORE PARALLEL RUN — %d categories, max %d concurrent",
                total, MAX_CONCURRENT)
    logger.info("Resume run: %d categories left after prior tunnel-drop cascade", total)
    logger.info("=" * 70)
    ...
    async with async_session_factory() as db:
        row = await db.execute(text("SELECT count(*) FROM activities WHERE city = 'Singapore'"))
        logger.info("\nTotal Singapore activities in DB: %d", row.scalar())
