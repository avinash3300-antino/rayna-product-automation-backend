"""Run the REAL pipeline for ALL 15 London categories sequentially.

Usage: python run_all_categories.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("all_categories")
...
async def main():
    ...
    async with async_session_factory() as db:
        row = await db.execute(text("SELECT count(*) FROM activities"))
        logger.info("\nTotal activities in DB: %d", row.scalar())
