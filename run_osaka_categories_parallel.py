"""Run the REAL pipeline for Osaka — PARALLEL variant with domain-diverse
source approval (per pipeline-source-approval memory).

15 Osaka-tailored categories: Universal Studios Japan (Osaka's biggest draw),
Osaka Castle, Dotonbori/Namba food scene, Kyoto/Nara/Himeji day trips.

Usage: python run_osaka_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("osaka_parallel")
...
ALL_CATEGORIES = [
    ...
    "Osaka Castle & Historic Sites",
    "Dotonbori & Namba Food Tours",
    "Day Trips",
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("OSAKA PARALLEL RUN — %d categories, max %d concurrent (domain-diverse approval)",
