"""Run the REAL pipeline for Langkawi — PARALLEL-3 variant.

Categories tailored for an island destination (cable car, island hopping,
mangrove, snorkeling, sunset cruises, duty-free, etc.).

Usage: python run_langkawi_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("langkawi_parallel")

LANGKAWI_CITY_ID = "3d77c332-f761-4d0d-b452-f27e89c61899"
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("LANGKAWI RESUME RUN — %d categories that failed overnight, max %d concurrent",
    ...
    try:
        async with async_session_factory() as db:
            row = await db.execute(text("SELECT count(*) FROM activities WHERE city = 'Langkawi'"))
            logger.info("\nTotal Langkawi activities in DB: %d", row.scalar())
    except Exception as exc:
