"""Run the REAL pipeline for Hanoi — PARALLEL variant with domain-diverse
source approval (baked in per pipeline-source-approval memory).

15 Hanoi-tailored categories — heavy focus on Halong Bay / Ninh Binh / Sapa
day trips (Hanoi's biggest tourism draws), Old Quarter walking, water puppets,
street food/cyclo tours.

Usage: python run_hanoi_categories_parallel.py
"""
import asyncio
import logging
import traceback
...
logger = logging.getLogger("hanoi_parallel")

HANOI_CITY_ID = "7678f28c-7d4b-4852-bab7-0d10e0aec47f"
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("HANOI PARALLEL RUN — %d categories, max %d concurrent (domain-diverse approval)",
    ...
    try:
        async with async_session_factory() as db:
            row = await db.execute(text("SELECT count(*) FROM activities WHERE city = 'Hanoi'"))
            logger.info("\nTotal Hanoi activities in DB: %d", row.scalar())
    except Exception as exc:
